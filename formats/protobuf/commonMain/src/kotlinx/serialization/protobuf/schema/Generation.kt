...
public fun generateProto2Schema(
    ...
    return generateProto2SchemaString(customTypes, packageName, options)
}
...
private fun generateMessage(
    ...
    for (index in 0 until serialDescriptor.elementsCount) {
        ...
        if (serialDescriptor.isElementOptional(index)) {
            builder.appendLine("  // WARNING: field has a default value that is not present in the schema")
            println(
                """WARNING: field '$fieldName' in serializable class '${serialDescriptor.serialName}' """ +
                        "has a default value that is not present in the schema!"
            )
        }
        ...
        try {
            ...
            // incorrect user input. Generation works fine, the problem is solved by changing user data
            ...
            // internal generation error. The problem is solved only by hotfix
...
private val SerialDescriptor.isProtobufNamedType: Boolean
    get() = isProtobufScalar || isProtobufCustomType
...
private val SerialDescriptor.isProtobufMessage: Boolean
    get() = isProtobufStaticMessage || isProtobufOpenMessage || isProtobufSealedMessage || isProtobufContextualMessage
...
private val SerialDescriptor.isProtobufStaticMessage: Boolean
    get() = kind == StructureKind.CLASS || kind == StructureKind.OBJECT
...
private val SerialDescriptor.isProtobufContextualMessage: Boolean
    get() = kind == SerialKind.CONTEXTUAL
...
private val SerialDescriptor.protobufCustomTypeName: String
    get() = makeProtobufIdent(serialName.substringAfterLast('.', serialName))
